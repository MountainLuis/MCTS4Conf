...
public class TreeFunction {
    ...
    public  List<TreeActionNode> computeChildren(TreeNode node,List<TreeActionNode> childrenList){
        List<TreeActionNode> mChildren = childrenList;
        mChildren.add(new TreeActionNode(null,node,node.getTree()));
        mChildren.addAll(computeAltActionChildren(node.getState1(),node,mChildren));
        mChildren.addAll(computeAltActionChildren(node.getState2(),node,mChildren));
        mChildren.addAll(computeSpdActionChildren(node.getState1(),node,mChildren));
        mChildren.addAll(computeSpdActionChildren(node.getState2(),node,mChildren));
        return mChildren;
    }

    private List<TreeActionNode> computeAltActionChildren(AcftState state, TreeNode node, List<TreeActionNode> childrenList){
        List<TreeActionNode> mChildren = childrenList;
...
//        }
    ...
    private List<TreeActionNode> computeSpdActionChildren(AcftState state, TreeNode node, List<TreeActionNode> childrenList){
        List<TreeActionNode> mChildren = childrenList;
        Aircraft acft = state.aircraft;
